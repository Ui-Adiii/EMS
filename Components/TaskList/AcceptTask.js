...
const AcceptTask = ({data,formatted,handleSubmit}) => {
  return (
    <div className="flex-shrink-0 p-5 h-full w-[300px] flex flex-col justify-between bg-yellow-400 rounded-xl">
        ...
        <div className='flex justify-between items-center mt-8 px-6 py-2'>
            <button onClick={() => handleSubmit(data._id, "completed")} className='px-4 py-[5px] bg-green-600 rounded-xl text-sm font-semibold '>completed</button>
            <button onClick={() => handleSubmit(data._id, "failed")} className='px-4 py-[5px] bg-red-600 rounded-xl text-sm font-semibold'>Failed</button>
        </div>
      </div>
  )
}
